...
function renderConfig(config) {
    // Global settings
    document.getElementById('max-parallel').value = config.global_settings?.max_parallel_experiments || 2;
    document.getElementById('output-dir').value = config.global_settings?.output_dir || './experiments_results';
    document.getElementById('log-level').value = config.global_settings?.log_level || 'INFO';
...
function updateOrganoidExperiment(idx, key, value) {
    ...
    if (!currentConfig.organoid_experiments[idx]) currentConfig.organoid_experiments[idx] = {};
...
function updateTILExperiment(idx, key, value) {
    ...
    if (!currentConfig.til_experiments[idx]) currentConfig.til_experiments[idx] = {};
...
async function saveConfig() {
    try {
        ...
        currentConfig.global_settings = {
            max_parallel_experiments: parseInt(document.getElementById('max-parallel').value),
...
function updateStatusDisplay(status) {
    ...
    document.getElementById('current-experiment').textContent = status.current || '-';
    document.getElementById('progress-text').textContent = `${status.completed}/${status.total}`;
...
function displayResults(results) {
    ...
    if (results.organoid && results.organoid.length > 0) {
        organoidEl.innerHTML = results.organoid.map(result => {
        ...
        organoidEl.innerHTML = '<div class="empty-state">No Organoid results yet</div>';
    }
    
    if (results.til && results.til.length > 0) {
        tilEl.innerHTML = results.til.map(result => {
            const date = new Date(result.timestamp * 1000).toLocaleString();
